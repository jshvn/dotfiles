...
| Top-level `git/` and `ssh/` v1 leftover directories (12 tracked files) — Phase 9 audit gap | closed in commits 76370b2 + dc9397d (2026-05-18) | Phase 4 ported identity to `identity/git/` and `identity/ssh/` but did not delete the v1 source dirs; Phase 9's v1-Drop Audit did not enumerate them; Phase 11 followed the audit faithfully and left them in place. One file (`ssh/configs/agent.toml` — 1Password SSH agent config) had no v2 equivalent and was ported into `identity/ssh/agent.toml` with a new `one-password-agent` task + `validate:one-password-agent` sub-task wired in `taskfiles/identity.yml` (feature-gated on `one-password-ssh`). One shell function (`shell/functions/sshlist.zsh`) still read the v1 path and was rewritten for the v2 identity overlay layout. Eight live operator symlinks were repointed: `~/.config/1Password/ssh/agent.toml` (now -> v2), and 7 stale/broken v1-pointing links (`~/.ssh/{cloudflared.zsh, config-personal, id_ed25519_personal.pub}` and `~/.config/git/{config-personal, personal/, server/, work/}`) were removed. `task install` + `task validate` + `task identity:validate` all exit 0 post-cleanup with the new "1Password SSH agent config linked" check passing. |
| `~/.config/claude/hooks` parent-symlink (Feb 2026, predates Phase 11) | remediated during Phase 11 verification | `~/.config/claude/hooks` was a single symlink to `dotfiles/claude/hooks` rather than a real directory with per-file symlinks underneath. Combined with `_:safe-link`'s "refuse to overwrite non-symlink targets" rule, this caused `task install` to fail at `links:claude` because individual file targets resolved through the parent symlink to regular files in the repo. The earlier session's first remediation attempt accidentally created self-referencing symlinks at the repo paths (corruption recovered via `git restore`). Correct fix: remove the parent symlink so `_:safe-link` can create `~/.config/claude/hooks/` as a real directory and populate it with per-file symlinks. Post-fix: `links:reconcile --warn-only` reports "no orphan symlinks detected." |
